...
import { Question, questionCategories } from 'app/questions/questions';
...
export class QuizComponent implements OnInit {
  ...
  questionCategories = Array.from(questionCategories.keys());

  constructor(private route: ActivatedRoute) {
    this.categories = this.route.snapshot.params["categories"].split(',');
    ...
    this.questions = this.categories.flatMap(c => {
      return questionCategories.get(c)!.questions.map(q => {
        q.categoryName = questionCategories.get(c)?.name;
        q.categoryColor = questionCategories.get(c)?.color;
        return q;
      });
    });

    this.currentQuestionIndex = this.getBiasedRandomQuestionId();

    this.currentQuestion$ = new BehaviorSubject<Question>(this.questions[this.currentQuestionIndex]);
  }


  ngOnInit(): void {
    const save = localStorage.getItem("streakMap-" + this.categories.join(",")) ?? "[]";
  ...
  private storeStreakMap() {
    localStorage.setItem("streakMap-" + this.categories.join(","), JSON.stringify(Array.from(this.streakMap.entries())));
  }
